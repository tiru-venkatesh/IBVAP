...
def assistant_chat(payload:schemas.AssistantChatRequest,db:Session=Depends(get_db)):
    m=payload.message.lower(); cams=db.query(models.Camera).count(); online=db.query(models.Camera).filter_by(status='Online').count(); alerts=db.query(models.Alert).order_by(models.Alert.id.desc()).all(); zones=db.query(models.FenceZone).all(); ev=db.query(models.AnalyticsEvent).count(); anpr=db.query(models.ANPRRecord).count(); faces=db.query(models.AnalyticsEvent).filter_by(event_type='face_match').count()
    if any(x in m for x in ['hello','hi','hey']): r='Hello Operator. IBVAP is online. I can report cameras, alerts, ANPR, faces, night movement, tracking and fence activity.'
    elif 'camera' in m: r=f'{online}/{cams} cameras are currently online.'
    elif 'alert' in m: r=f'{len(alerts)} alerts are logged. '+(f'Most recent: {alerts[0].title} ({alerts[0].severity}).' if alerts else 'No alerts are logged.')
    elif 'plate' in m or 'anpr' in m: r=f'{anpr} ANPR reads are logged.'
    elif 'face' in m: r=f'{faces} face-match analytics events are logged; face detection is available on every analyzed image/video frame.'
    elif 'night' in m or 'dark' in m: r=f'{db.query(models.NightEvent).count()} low-light events are logged.'
    elif 'fence' in m or 'intrusion' in m: r=f'{len(zones)} logical fence zones are configured and {db.query(models.AnalyticsEvent).filter_by(event_type="intrusion").count()} intrusion events are logged.'
    elif 'status' in m or 'system' in m: r=f'IBVAP online: {online}/{cams} cameras, {len(alerts)} alerts, {anpr} ANPR reads, {ev} analytics events.'
    else: r='I can summarize cameras, alerts, ANPR, face activity, night movement, fence intrusions and analytics events.'
    return schemas.AssistantChatResponse(reply=r)
